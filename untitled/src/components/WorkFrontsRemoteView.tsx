...
import { Wrench, Search, RefreshCw, MapPin, Calendar, UserCheck, Trash2, AlertCircle, Loader2, Boxes, Truck, Play, CheckCircle2 } from 'lucide-react';
...
export const WorkFrontsView: React.FC<WorkFrontsViewProps> = ({ onNavigateToMaterials, onNavigateToDeliveries }) => {
  ...
  const canDelete = isCoordinator(profile?.role) || isManagement(profile?.role);
  ...
  const activateFront = async (front: WorkFrontRecord) => {
    ...
    const ok = window.confirm(`¿Confirmas que hoy inician actividades en el frente ${front.frontCode}?\n\nEl frente pasará a EN EJECUCIÓN.`);
    if (!ok) return;
    setBusyId(front.id);
    setError(null);
    try {
      await updateWorkFrontStatusInDb(front.id, 'EN EJECUCIÓN');
      setNotice(`Frente ${front.frontCode} activado. Ya puedes generar solicitudes de materiales.`);
      await load();
      window.setTimeout(() => setNotice(null), 4500);
    } catch (e: any) {
      setError(e?.message || 'No se pudo activar el frente.');
    } finally {
      setBusyId(null);
    }
  };

  const goMaterials = (front: WorkFrontRecord) => {
    sessionStorage.setItem('sipre_selected_material_front', front.id);
    onNavigateToMaterials?.();
  };

  const remove = async (front: WorkFrontRecord) => {
    if (!canDelete) return;
    const ok = window.confirm(`¿Eliminar definitivamente el frente ${front.frontCode}?\n\nÚsalo solamente para borrar una prueba o dato creado por error.`);
    if (!ok) return;
    setBusyId(front.id);
    try {
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-5">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-xl flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">Después de la aprobación, el frente queda PENDIENTE. Solo el profesional responsable puede activarlo cuando realmente comiencen las actividades.</p>
        </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 flex flex-col sm:flex-row gap-3 sm:items-center">
        ...
        {professional && <label className="flex items-center gap-2 text-xs text-slate-300 font-bold bg-slate-950 px-3 py-2 rounded-xl border border-slate-800"><input type="checkbox" checked={onlyMine} onChange={e=>setOnlyMine(e.target.checked)}/>Solo frentes a mi cargo</label>}
      ...
      {loading ? <div className="bg-slate-900 border border-slate-800 rounded-2xl p-10 text-center text-slate-400"><Loader2 className="w-5 h-5 animate-spin mx-auto mb-2"/>Cargando frentes desde Supabase...</div> : filtered.length === 0 ? (
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-10 text-center"><Wrench className="w-8 h-8 text-slate-600 mx-auto mb-2"/><h3 className="text-sm font-bold text-white">No hay frentes remotos para mostrar</h3><p className="text-xs text-slate-400 mt-1">Los frentes aparecen aquí después de una intervención aprobada.</p></div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {filtered.map(front => {
            const mine = isMine(front);
            return <div key={front.id} className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-lg space-y-4">
              <div className="flex items-start justify-between gap-3">
                ...
                {canDelete && <button disabled={busyId===front.id} onClick={()=>remove(front)} title="Eliminar prueba o dato erróneo" className="p-2 rounded-lg bg-red-950/60 border border-red-900 text-red-300 hover:bg-red-900 disabled:opacity-50"><Trash2 className="w-4 h-4"/></button>}
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                ...
                <div className="bg-slate-950 rounded-xl p-3 border border-slate-800"><div className="text-slate-500 font-bold">PROGRAMACIÓN</div><div className="text-slate-200 mt-1 flex items-center gap-1"><Calendar className="w-3.5 h-3.5 text-cyan-400"/>{front.plannedStartDate || 'Por programar'} → {front.plannedCompletionDate || 'Por programar'}</div></div>
              </div>
              ...
              <div className="flex items-center justify-between gap-2 border-t border-slate-800 pt-3"><div><div className="text-[10px] text-slate-500 font-bold">ESTADO</div><div className="font-black text-sm text-cyan-500 mt-0.5">{front.status}</div></div><div className="flex gap-2">
                {professional && mine && front.status === 'PENDIENTE' && <button disabled={busyId===front.id} onClick={() => activateFront(front)} className="px-3 py-2 rounded-lg bg-emerald-600 text-white text-xs font-bold flex items-center gap-1">{busyId===front.id ? <Loader2 className="w-4 h-4 animate-spin"/> : <Play className="w-4 h-4"/>}ACTIVAR FRENTE</button>}
                {professional && mine && front.status === 'EN EJECUCIÓN' && <button onClick={() => goMaterials(front)} className="px-3 py-2 rounded-lg bg-cyan-600 text-white text-xs font-bold flex items-center gap-1"><Boxes className="w-4 h-4"/>Solicitar materiales</button>}
                {front.status === 'ENTREGADO' || front.status === 'CERRADO' ? <CheckCircle2 className="w-5 h-5 text-emerald-500"/> : null}
              </div></div>
            </div>;
          })}
        </div>
