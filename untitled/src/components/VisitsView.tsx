...
export const VisitsView: React.FC<VisitsViewProps> = ({ onOpenScheduleVisitModal, onStartFieldMode }) => {
  ...
  const visible = useMemo(() => {
    if (professional) return visits.filter(isAssignedToCurrentUser);
    if (filter === 'mine') return visits.filter(isAssignedToCurrentUser);
    return visits;
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-5">
      ...
      {!professional && (
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-2 flex gap-2">
          <button onClick={() => setFilter('all')} className={`px-4 py-2 rounded-xl text-xs font-bold ${filter === 'all' ? 'bg-cyan-600 text-white' : 'text-slate-400 hover:bg-slate-800'}`}><Eye className="w-4 h-4 inline mr-1" />TODAS</button>
          <button onClick={() => setFilter('mine')} className={`px-4 py-2 rounded-xl text-xs font-bold ${filter === 'mine' ? 'bg-cyan-600 text-white' : 'text-slate-400 hover:bg-slate-800'}`}><UserCheck className="w-4 h-4 inline mr-1" />ASIGNADAS A MÍ</button>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-10 text-center text-slate-400">No hay visitas visibles para este usuario.</div>
      ) : (
