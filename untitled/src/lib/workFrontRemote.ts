...
export async function getWorkFrontsFromDb(): Promise<WorkFrontRecord[]> {
  ...
  if (!client) return [];
  const { data: rows, error } = await client.from('work_fronts').select('*').order('created_at', { ascending: false });
  ...
  const fronts: WorkFrontRecord[] = (rows || []).map((r: any) => {
    ...
    return {
      ...
      progressCategory: r.progress_category || (uiStatus(r.status) === 'EN EJECUCIÓN' ? 'En proceso' : ['ENTREGADO','CERRADO'].includes(uiStatus(r.status)) ? 'Completado' : 'No iniciado'),
...
export async function syncWorkFrontCacheFromDb(): Promise<WorkFrontRecord[]> {
  try { return await getWorkFrontsFromDb(); }
  catch (err) { console.warn('Work-front cache sync error:', err); return []; }
}
...
export async function createWorkFrontFromVisitInDb(input: any): Promise<WorkFrontRecord> {
  ...
  const { data: caseRow } = await client.from('cases').select('id, case_code, property_id, client_name, address').eq('id', input.caseId).maybeSingle();

  let professionalId = input.responsibleTechnicalProfessionalId || null;
  if (!professionalId && input.responsibleTechnicalProfessional) {
    const { data: profileRow } = await client.from('profiles').select('id').ilike('full_name', input.responsibleTechnicalProfessional.trim()).limit(1).maybeSingle();
    professionalId = profileRow?.id || null;
  }
  if (!professionalId && input.visitId) {
    const { data: visitRow } = await client.from('visits').select('assigned_to').eq('id', input.visitId).maybeSingle();
  ...
  return {
    id: data.id, frontCode, caseId: input.caseId, caseCode: caseRow?.case_code || input.caseCode || '', visitId: input.visitId,
    propertyAddress: caseRow?.address || input.propertyAddress || '', clientName: caseRow?.client_name || input.clientName || '', repairScope: payload.scope_description,
    responsibleTechnicalProfessional: input.responsibleTechnicalProfessional || 'Por asignar', fieldSupervisor: input.fieldSupervisor || 'Por asignar',
    plannedStartDate: input.plannedStartDate || '', plannedCompletionDate: input.plannedCompletionDate || '', status: 'PENDIENTE', progressCategory: 'No iniciado', createdAt: now, updatedAt: now,
  };
}
...
export async function updateWorkFrontStatusInDb(id: string, status: WorkFrontStatus): Promise<void> {
  const client = getSupabaseClient();
  if (!client) throw new Error('Supabase no está configurado.');
  const { data, error } = await client.from('work_fronts').update({ status: dbStatus(status), updated_at: new Date().toISOString() }).eq('id', id).select('id');
  if (error) throw new Error(error.message);
  if (!data?.length) throw new Error('Supabase no permitió actualizar el frente. Revisa los permisos RLS del usuario.');
...
export async function deleteWorkFrontInDb(id: string): Promise<void> {
  ...
  // Clean test/audit rows tied to this front, including remote material requests.
  const { error: activityError } = await client.from('activity_log').delete().eq('work_front_id', id);
  if (activityError) throw new Error(`No se pudieron limpiar los registros remotos del frente: ${activityError.message}`);
  ...
  if (rpcError) {
    ...
    const { data, error } = await client.from('work_fronts').delete().eq('id', id).select('id');
    if (error) throw new Error(error.message);
    if (!data?.length) throw new Error('Supabase no permitió borrar el frente. Revisa los permisos RLS del usuario.');
  }

  const { data: stillThere, error: verifyError } = await client.from('work_fronts').select('id').eq('id', id).maybeSingle();
  if (verifyError && !String(verifyError.message || '').toLowerCase().includes('multiple')) throw new Error(verifyError.message);
  if (stillThere) throw new Error('El frente sigue existiendo después del intento de borrado. Los permisos RLS están bloqueando la eliminación.');
  await getWorkFrontsFromDb();
}
...
export function subscribeWorkFrontsRealtime(onChange: () => void): () => void {
  const client = getSupabaseClient();
  if (!client) return () => {};
  const channel = client.channel(`sipre-work-fronts-${Math.random().toString(36).slice(2)}`).on('postgres_changes', { event: '*', schema: 'public', table: 'work_fronts' }, onChange).subscribe();
